    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-sm p-6">
        ...
        <div className="flex gap-2 mt-5">
          <button onClick={onCancel} className="flex-1 px-4 py-2 text-xs font-medium border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors">{cancelLabel || 'Cancel'}</button>
          <button onClick={onConfirm} className={`flex-1 px-4 py-2 text-xs font-semibold text-white rounded-lg transition-colors ${confirmClass || 'bg-red-600 hover:bg-red-700'}`}>{confirmLabel || 'Confirm'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-sm p-6">
        <div className="flex flex-col items-center text-center gap-3">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">You have unsaved changes. Do you want to save before leaving, or discard them?</p>
        </div>
        <div className="flex gap-2 mt-5">
          ...
          <button onClick={onSave} className="flex-1 px-4 py-2 text-xs font-semibold bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors">Save & leave</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex items-start gap-2 p-3 bg-gray-50 rounded-xl border border-gray-100 group">
      ...
      <div className="flex-1 grid grid-cols-2 gap-2 min-w-0">
        <div><label className="text-[10px] text-gray-400 mb-1 block">Name</label>
          <input value={variant.variant_name ?? ''} onChange={e => onChange({ ...variant, variant_name: e.target.value })} className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:border-gray-400" /></div>
        <div><label className="text-[10px] text-gray-400 mb-1 block">SKU</label>
          <input value={variant.variant_sku ?? ''} onChange={e => onChange({ ...variant, variant_sku: e.target.value })} className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:border-gray-400 font-mono" /></div>
        <div><label className="text-[10px] text-gray-400 mb-1 block">Price</label>
          <input type="number" step="0.01" value={variant.price ?? ''} onChange={e => onChange({ ...variant, price: e.target.value })} className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:border-gray-400" /></div>
        <div><label className="text-[10px] text-gray-400 mb-1 block">Stock</label>
          <input type="number" value={variant.stock ?? ''} onChange={e => onChange({ ...variant, stock: parseInt(e.target.value) || 0 })} className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:border-gray-400" /></div>
        {variant.option1_name && (
          <div className="col-span-2"><label className="text-[10px] text-gray-400 mb-1 block">{variant.option1_name}</label>
            <input value={variant.option1_value ?? ''} onChange={e => onChange({ ...variant, option1_value: e.target.value })} className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:border-gray-400" /></div>
        )}
      </div>
      <button onClick={onDelete} className="shrink-0 mt-1 p-1.5 text-gray-300 hover:text-red-500 transition-colors rounded-lg hover:bg-red-50"><Trash2 size={13} /></button>
    </div>
  )
}
...
export default function ProductEditPage({ productId, suppliers, categories, onBack, onSaved }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (p) {
        ...
        const ovRow = await api.get(`/api/product-overrides/${encodeURIComponent(p.sku)}`)
        const hasOv = !!ovRow

        setHasOverride(hasOv)
        const pfData = {
          product_id: p.product_id, sku: p.sku, price: p.price, stock: p.stock,
          category: p.category, brand: p.brand, short_description: p.short_description,
          product_url: p.product_url, product_type: p.product_type,
          supplier_id: p.supplier_id, created_at: p.created_at, updated_at: p.updated_at,
        }
        const ovData = {
  ...
  async function doRemoveOverride() {
    setShowRemoveOverride(false); setError(null)
    await api.delete(`/api/product-overrides/${encodeURIComponent(pf.sku)}`)
    const p = originalProductRef.current
    if (p) {
      const rawImages = Array.isArray(p.images) ? p.images
        : typeof p.images === 'string' ? (() => { try { return JSON.parse(p.images) } catch { return [] } })() : []
      const restoredOv = { title: p.title ?? '', description: p.description ?? '', images: rawImages }
      setOv(restoredOv); setOvSnap(JSON.parse(JSON.stringify(restoredOv)))
    }
    setHasOverride(false); setSaved(true); setTimeout(() => setSaved(false), 2500); onSaved?.()
  }
  ...
  async function handleSave() {
    ...
    try {
      await api.put(`/api/products/${productId}`, {
        title: ov.title, price: parseFloat(pf.price) || 0, stock: parseInt(pf.stock) || 0,
        description: ov.description, short_description: pf.short_description,
        category: pf.category, brand: pf.brand, images: ov.images,
        product_url: pf.product_url, metadata: {}, supplier_id: pf.supplier_id,
      })
      await api.post('/api/product-overrides', {
        sku: pf.sku, title: ov.title || null,
        description: ov.description || null, images: ov.images ?? [],
      })
      setHasOverride(true)
  ...
  async function handleSaveAndLeave() { await handleSave(); setShowUnsaved(false); onBack() }
  ...
  if (!pf || !ov) return <div className="p-8 text-center text-gray-400">Product not found.</div>

  const dirty = isDirty()

  return (
    <div className="min-h-full bg-gray-50">
      <ConfirmModal open={showRemoveOverride} icon={<Trash2 size={22} />} title="Remove override?"
        message="This will delete the override record and restore the original scraped data."
        confirmLabel="Yes, remove override" confirmClass="bg-purple-600 hover:bg-purple-700"
        cancelLabel="Keep override" onConfirm={doRemoveOverride} onCancel={() => setShowRemoveOverride(false)} />
      <UnsavedModal open={showUnsaved} onSave={handleSaveAndLeave}
        onDiscard={() => { setShowUnsaved(false); onBack() }} onCancel={() => setShowUnsaved(false)} />
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button onClick={handleBack} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-700 transition-colors">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            {hasOverride && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-purple-50 text-purple-700 border border-purple-100 whitespace-nowrap">🔒 Manually edited</span>}
            {dirty && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-amber-50 text-amber-600 border border-amber-100 whitespace-nowrap">● Unsaved</span>}
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {saved && <div className="flex items-center gap-1.5 text-xs text-green-600 bg-green-50 px-3 py-1.5 rounded-lg border border-green-100"><Check size={12} /> Saved</div>}
          {hasOverride && <button onClick={() => setShowRemoveOverride(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 transition-colors">🔓 Remove override</button>}
          {pf.product_url && <a href={pf.product_url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-gray-200 text-gray-500 rounded-lg hover:bg-gray-50 transition-colors"><ExternalLink size={12} /> View store</a>}
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-semibold bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors disabled:opacity-50">
        ...
        <div className="grid grid-cols-3 gap-5">
          <div className="col-span-2 space-y-5">
            <Section title="Product Info" icon={Pencil}>
              <div className="space-y-4">
                <Field label="Title" hint="Saved to product_overrides"><Input value={ov.title} onChange={v => setOvField('title', v)} placeholder="Product title" /></Field>
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Brand" hint="Saved to products"><Input value={pf.brand} onChange={v => setPfField('brand', v)} placeholder="Brand name" /></Field>
                  <Field label="SKU" hint="Saved to products"><Input value={pf.sku} onChange={v => setPfField('sku', v)} placeholder="SKU" className="font-mono" /></Field>
                </div>
                <Field label="Description" hint="Saved to product_overrides"><Textarea value={ov.description} onChange={v => setOvField('description', v)} placeholder="Full product description…" rows={5} /></Field>
                <Field label="Short Description" hint="Saved to products"><Textarea value={pf.short_description} onChange={v => setPfField('short_description', v)} placeholder="Brief summary…" rows={2} /></Field>
                <Field label="Product URL" hint="Saved to products"><Input value={pf.product_url} onChange={v => setPfField('product_url', v)} placeholder="https://…" type="url" /></Field>
              </div>
            </Section>
            <Section title="Images" icon={ImagePlus}><ImageManager images={ov.images} onChange={v => setOvField('images', v)} /></Section>
            {pf.product_type === 'variation_parent' && (
              <Section title="Variants" icon={Layers}>
                {variants.length === 0
                  ...
                  : <div className="space-y-2">{variants.map((v, i) => (
          ...
          <div className="space-y-5">
            <Section title="Pricing & Stock" icon={Tag}>
              <div className="space-y-4">
                <Field label="Price ($)"><Input type="number" value={pf.price} onChange={v => setPfField('price', v)} placeholder="0.00" /></Field>
                <Field label="Stock"><Input type="number" value={pf.stock} onChange={v => setPfField('stock', v)} placeholder="0" /></Field>
                <div className="pt-1"><StockBadge stock={parseInt(pf.stock) || 0} /></div>
              </div>
            </Section>
              ...
              <div className="space-y-4">
                ...
                <Field label="Supplier">
                  <select value={pf.supplier_id || ''} onChange={e => setPfField('supplier_id', parseInt(e.target.value))}
                    className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-gray-400 text-gray-800 bg-white">
                    <option value="">— None —</option>
                    {Object.values(suppliers).map(s => <option key={s.supplier_id} value={s.supplier_id}>{s.supplier_name}</option>)}
                  </select>
                </Field>
                <Field label="Product Type"><Input value={pf.product_type || ''} onChange={v => setPfField('product_type', v)} placeholder="e.g. variation_parent" /></Field>
              </div>
            </Section>
            <div className="bg-gray-50 rounded-2xl border border-gray-100 p-4 space-y-2">
              ...
              <div className="space-y-1 text-xs text-gray-400">
                {[['Product ID', pf.product_id], ['SKU', pf.sku],
                  ['Created', pf.created_at ? new Date(pf.created_at).toLocaleDateString() : '—'],
                  ['Updated', pf.updated_at ? new Date(pf.updated_at).toLocaleDateString() : '—'],
                ].map(([k, v]) => (
                  <div key={k} className="flex justify-between"><span>{k}</span><span className="font-mono text-gray-600">{v}</span></div>
                ))}
                ...
                <div className="flex justify-between pt-1 border-t border-gray-100 mt-1">
                  <span>Override</span>
                  <span className={hasOverride ? 'text-purple-600 font-medium' : 'text-gray-300'}>{hasOverride ? 'Active' : 'None'}</span>
                </div>
              </div>
            </div>
          </div>
